...
# REQUEST MODEL
# Contains same data as the SPECIAL MODEL, but for storing unapproved requests to add deals that users send in.
...
class Request(models.Model):
    ...
    # Note: "related_name" is what "each special is to a given restaurant."
    # Usually it is just the lower-cased plural of the model it is within.
    # Note 2: the "to_field='name'" arg makes it so that the Query returns the actual name of the restaurant (b/c the Restaurant's name field is 'name), rather than the restaurant's pk by default. 
    restaurant = models.ForeignKey(Restaurant, related_name="requests", to_field='name', on_delete=models.CASCADE)
